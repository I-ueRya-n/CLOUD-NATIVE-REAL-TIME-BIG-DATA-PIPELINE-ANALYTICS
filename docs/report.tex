...
\subsection{Reddit Data Processing Architecture} % Reddit 資料處理架構
...
\begin{enumerate}
    \item \textbf{Entrypoint Functions (Initiating Data Collection):} % 進入點函數 (啟動資料收集)
    ...
    \item \textbf{Data Queuing (Redis):} % 資料佇列 (Redis)
    ...
    \item \textbf{Data Indexing into Elasticsearch (Adder Function):} % 資料索引到 Elasticsearch (添加器函數)
