...
words = [
    "aus", "australia", "aussie", "victoria", "canberra", "sydney",
    "politics", "pol", "labor", "liberal"
]
...
def worker_main(cursor_value: multiprocessing.Value, pool_queue: multiprocessing.Queue) -> None:
    ...
    while True:
        ...
        for created_post in ops[models.ids.AppBskyFeedPost]['created']:
            ...
            if record is None or record.tags is None:
                continue

            if not relevant_text(record.text):
                continue
...
def measure_events_per_second(func: callable) -> callable:
    def wrapper(*args) -> Any:
        ...
        if cur_time - wrapper.start_time >= 60:
            print(f'NETWORK LOAD: {wrapper.calls} events/minute')
